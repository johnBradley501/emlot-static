...
def processDateData(record, start):
    ...
    to_month, to_season, to_year) = record[start:start+len(date_fields)]
    if undated:
        return "undated"
...
def fetchTitleNames(cursor):
    rslt={}
    cursor.execute("SELECT id, name FROM ltbapp_persontitle")
    for r in cursor:
        (id, name) = r
        id = int(id)
        rslt[id] = name
...
titles = fetchTitleNames(cursor);


cursor.execute("SELECT id, "+makeDateFields("")+", surname, name, title_id, helper_keywordsearch FROM ltbapp_person order by surname, name") # WHERE review=1") pehaps this should be editedrecord?
persons = []
cnt = 0
for r in cursor:
    cnt += 1
    ...
    title = "..."
    title_id = r[3 + len(date_fields)]
    if title_id:
        title = titles[title_id]
    rest = list(r[1+len(date_fields):])
    rest[2] = title
    items.extend(rest)
...
outfile.write("function getPersonsData(){ return")
outfile.write(json.dumps(persons)) # , indent=4))
outfile.write("; }")
outfile.close()
