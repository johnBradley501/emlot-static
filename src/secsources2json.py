...
cursor.execute("SELECT id, "+makeDateFields("pubdate_")+", "+makeDateFields("str_ent_")+", title, fulltitle, helper_keywordsearch FROM ltbapp_document where document_metatype_id = 3 order by title") # WHERE review=1") pehaps this should be editedrecord?
sources = []
cnt = 0
for r in cursor:
    cnt += 1
...
outfile.write("function getSecondarySourceData(){ return")
